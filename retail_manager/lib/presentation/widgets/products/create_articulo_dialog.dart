...
/// Diálogo de formulario para crear nuevos artículos (variantes de producto)
class CreateArticuloDialog extends StatefulWidget {
  final ProductoMaster product;
  final List<ColorData> availableColors;
  final List<ColorData>? coloresUnicos;

  const CreateArticuloDialog({
    super.key,
  ...
  @override
  State<CreateArticuloDialog> createState() => _CreateArticuloDialogState();
}

class _CreateArticuloDialogState extends State<CreateArticuloDialog> {
  ...
  List<ColorData> _coloresUnicos = [];

  @override
  void initState() {
    super.initState();

    // Inicializar precio con el precio sugerido del producto
    _precioController.text = widget.product.precioSugerido.toStringAsFixed(2);

    // Inicializar colores únicos si se proporcionan
    _coloresUnicos = widget.coloresUnicos ?? [];

    // Cargar colores únicos si no se proporcionaron
  ...
  Future<void> _createArticulo() async {
    ...
    try {
      ...
      // Verificar que el color seleccionado esté disponible
      final selectedColorExists = widget.availableColors.any((c) => c.id == _selectedColorId);
      if (!selectedColorExists) {
        _showErrorMessage('El color seleccionado no está disponible');
        return;
      }

      // Validar que no exista duplicado (verificación adicional)
      ...
      final articuloData = {
        'producto_master_id': widget.product.id,
      ...
      final newArticulo = await _repository.createArticulo(articuloData);

      if (mounted) {
        Navigator.of(context).pop(newArticulo);
      }

    } catch (e) {
      String errorMessage = 'Error al crear artículo';
      ...
      if (e.toString().contains('23505')) {
        ...
        errorMessage = 'Error al crear artículo: ${e.toString().substring(0, 100)}';
      }

      print('❌ [CreateArticuloDialog] Error: $e');
  ...
  // Método para cargar colores únicos
                              ...
                              SegmentedButton<String>(
                                ...
                                onSelectionChanged: (Set<String> selected) {
                                  setState(() {
                                    ...
                                    // Reset selección si el color actual no está en el filtro
                                    if (_selectedColorId != null) {
                                      final filteredColors = _getFilteredColors();
                                      if (!filteredColors.any((c) => c.id == _selectedColorId)) {
                            ...
                            children: [
                              Container(
                                ...
                                child: ClipRRect(
                                  ...
                                  child: _getSelectedColor()?.esColorUnico == true
                                      ? Container(
                                          color: _getColorFromHex(_getSelectedColor()?.hexColor),
                                        )
                                      : _buildMultiColorPreview(_getSelectedColor()!),
                                ),
                              ),
                                          ...
                                          children: [
                                            Container(
                                              ...
                                              child: ClipRRect(
                                                ...
                                                child: Stack(
                                                  children: [
                                                    // Visualización correcta para colores múltiples
                                                    color.esColorUnico
                                                        ? Container(
                      ...
                      Container(
                        ...
                        child: Row(
                          children: [
                            ...
                            Expanded(
                              child: Text(
                                'Se creará un nuevo artículo con un SKU automático y se agregará al catálogo.',
                                style: TextStyle(
                                  fontSize: 12,
                                  color: Colors.blue[700],
                                ),
            ...
            Container(
              ...
              child: Row(
                ...
                children: [
                  ...
                  const SizedBox(width: 12),
                  ElevatedButton(
                    onPressed: _isLoading ? null : _createArticulo,
                    child: _isLoading
                        ? const SizedBox(
                            width: 20,
                            height: 20,
  ...
  ColorData? _getSelectedColor() {
    if (_selectedColorId == null) return null;
    return widget.availableColors.firstWhere(
  ...
  void _showPriceInfo() {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text(
          'Este es el precio sugerido para el nuevo artículo. '
